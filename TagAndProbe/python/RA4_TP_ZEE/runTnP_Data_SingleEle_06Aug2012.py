...
process.configurationMetadata = cms.untracked.PSet(
    ...
    name = cms.untracked.string('$Source: /RA4Efficiencies/TagAndProbe/python/RA4_TP_ZEE/runTnP_Data.py,v $'),
...
options.register('GlobalTag', "FT_53_V6C_AN2::All", VarParsing.VarParsing.multiplicity.singleton, VarParsing.VarParsing.varType.string, "GlobalTag to use (if empty default Pat GT is used)")
if options.mcInfo:
...
options.register('hltSelection', 'HLT_Ele22_CaloIdL_CaloIsoVL_v*', VarParsing.VarParsing.multiplicity.list, VarParsing.VarParsing.varType.string, "hlTriggers (OR) used to filter events")
options.hltSelection.append(     'HLT_Ele27_CaloIdL_CaloIsoVL_v*'                               )
options.hltSelection.append(     'HLT_Ele27_CaloIdL_CaloIsoVL_TrkIdVL_TrkIsoVL_v*'              )
options.hltSelection.append(     'HLT_Ele27_WP80_v*'                                            )
options.hltSelection.append(     'HLT_Ele27_WP80_PFMET_MT50_v*'                                 )
options.hltSelection.append(     'HLT_Ele27_WP80_CentralPFJet80_v*'                             )
options.hltSelection.append(     'HLT_Ele27_WP80_WCandPt80_v*'                                  )
options.hltSelection.append(     'HLT_Ele30_CaloIdVT_TrkIdT_v*'                                 )
options.hltSelection.append(     'HLT_Ele32_CaloIdL_CaloIsoVL_TrkIdVL_TrkIsoVL_v*'              )
options.hltSelection.append(     'HLT_Ele65_CaloIdVT_TrkIdT_v*'                                 )
options.hltSelection.append(     'HLT_Ele80_CaloIdVT_TrkIdT_v*'                                 )
options.hltSelection.append(     'HLT_Ele80_CaloIdVT_GsfTrkIdT_v*'                              )
options.hltSelection.append(     'HLT_Ele90_CaloIdVT_GsfTrkIdT_v*'                              )
options.hltSelection.append(     'HLT_Ele100_CaloIdVT_TrkIdT_v*'                                )
#options.hltSelection.append(     'HLT_Ele27_WP80_v*'                                            ) # Conflicts with other triggers, should only be selected alone
...
options.files.append('file:/data/jkarancs/data/SingleElectron__Run2012A-13jul2012-v1__AOD/96F5BE46-07D4-E111-BD1A-003048678FFA.root')
...
process.electronTriggerMatchHLTElectrons = cms.EDProducer("PATTriggerMatcherDRDPtLessByR",
    ...
    matchedCuts = cms.string('type( "TriggerElectron" ) && ('
                             ...
                             'path("HLT_Ele27_CaloIdL_CaloIsoVL_TrkIdVL_TrkIsoVL_v*"                         ,1,0) || '
                             'path("HLT_Ele30_CaloIdVT_TrkIdT_v*"                                            ,1,0) || '
                             'path("HLT_Ele32_CaloIdL_CaloIsoVL_TrkIdVL_TrkIsoVL_v*"                         ,1,0) || '
                             'path("HLT_Ele65_CaloIdVT_TrkIdT_v*"                                            ,1,0) || '
                             'path("HLT_Ele80_CaloIdVT_TrkIdT_v*"                                            ,1,0) || '
                             'path("HLT_Ele80_CaloIdVT_GsfTrkIdT_v*"                                         ,1,0) || '
                             'path("HLT_Ele90_CaloIdVT_GsfTrkIdT_v*"                                         ,1,0) || '
                             'path("HLT_Ele100_CaloIdVT_TrkIdT_v*"                                           ,1,0) '
                             #'path("HLT_Ele27_WP80_v*"                                                       ,1,0) ' # Conflicts with other triggers, should only be selected alone
                             ')'),
...
process.TFileService = cms.Service("TFileService", fileName = cms.string("tpZEE_Data.root"))
